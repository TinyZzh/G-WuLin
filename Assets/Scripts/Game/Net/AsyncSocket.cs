...
namespace Assets.Scripts.Game.Net
{
    ...
    public class AsyncSocket
    {
        ...
        ///     初始化Socket, 并设置帧长度
        /// </summary>
        /// <param name="encoderLengthFieldLength">
        ///     编码是消息长度数字的字节数长度. 1：表示1byte  2：表示2byte[Short类型] 4：表示4byte[int类型]
        ///     8：表示8byte[long类型]
        /// </param>
        /// <param name="decoderLengthFieldLength">
        ///     解码时消息长度数字的字节数长度. 1：表示1byte  2：表示2byte[Short类型] 4：表示4byte[int类型]
        ///     8：表示8byte[long类型]
        /// </param>
        public AsyncSocket(int encoderLengthFieldLength, int decoderLengthFieldLength) : this()
        {
        ...
        private void StartReceive()
        {
            ...
            _socket.BeginReceive(buffer, 0, _decoderLengthFieldLength, SocketFlags.None, OnReceiveFrameLengthComplete,
                buffer);
        }
        ...
        #region Protocol Buffers Utility 

#if GOOGLE_PROTOCOL_BUFFERS

        public void OnSendMessage<T>(T message)
            where T : IMessage
        {
            OnSendMessage(message.ToByteArray());
        }

#endif


#if PROTOBUF_CAHARP_2_X /// <summary>
/// 发送消息
